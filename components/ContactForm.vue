  ...
  methods: {
    async onSubmit() {
      ...
      try {
        ...
        const response = await this.$http.$get("https://us-central1-itsmeromian.cloudfunctions.net/sendMessage?" + "firstName=" + this.data.firstName + "&email=" + this.data.email + "&message=" + this.data.message + "&lastName=" + this.data.lastName + "&token=" + token);
        if (response.success) {
