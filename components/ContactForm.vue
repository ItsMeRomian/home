<template>
  <form @submit.prevent="onSubmit()">
    ...
    <div class="row pb-4 pt-3 px-4 rounded d-md-none" style="background: var(--secondary)">
      <div class="col-md-6 col-12">
    ...
    <div class="row mt-4 d-none d-md-flex" style="background: var(--secondary)">
      <div class="col">
        <div class="col p-3 rounded" >
          <input type="email" class="form-control form-control-lg" aria-label="Last name" placeholder="Your Email" v-model="data.email" />
        </div>
        <div class="col ">
          <button class="btn btn-success w-100 p-3 submit-button mb-3">{{ state }}</button>
        </div>
      </div>
      <div class="col">
        <div class="col p-3 rounded h-100" >
          <textarea class="form-control" aria-label="With textarea" placeholder="Your Message" v-model="data.message"></textarea>
        </div>
      </div>
    </div>
  ...
  methods: {
    async onSubmit() {
      ...
      try {
        ...
        const response = await this.$http.$get("https://us-central1-itsmeromian.cloudfunctions.net/sendDiscord?" + "firstName=" + this.data.firstName + "&email=" + this.data.email + "&message=" + this.data.message + "&lastName=" + this.data.lastName + "&token=" + token);
        if (response.success) {
